...
class Repository:
    ...
    def verify_files(self, files):
        for file in files:
            ...
            if not all(
                key in file for key in ["name", "md5", "num_segments", "owned_segments"]
            ):
                print(file)
            ...
            if any(
                existing_file["num_segments"] != file["num_segments"]
                for existing_file in existing_files
            ):
                raise FileFormatException(
                    "File with same name and md5 but different num_segments exists in the database"
                )
    ...
    def get_files_by_name_and_md5(self, name, md5):
        ...
        for client in self.db.values():
            result.extend(
                [file for file in client if file["name"] == name and file["md5"] == md5]
            )
        return result
        ...
        for client_address, files in self.db.items():
            for file in files:
                if (
                    file["name"] == name
                    and file["md5"] == md5
                    and segment_number in file["owned_segments"]
                ):
